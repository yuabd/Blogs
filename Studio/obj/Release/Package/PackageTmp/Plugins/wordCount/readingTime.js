...
;(function($) {
    ...
    $.fn.readingTime = function(options) {
	    ...
	    //default lang is english
	        ...
	        var lessThanAMinute = lessThanAMinuteString || '少于一分钟';
		...
		function countByteLength(str) {
            ...
            for (var i = 0; i < str.length; i++) {
                //alert（str.charAt（i））;
                //if (str.charAt(i).charCodeAt(0) > 256)
		...
		function reHtml(str) {  
			var reTag = /<(?:.|\s)*?>/g;
			//alert(str);
			return str.replace(" ", "").replace(reTag,"");  
		}

        var setTime = function(text) {

	        //split text by spaces to define total words  text.split(' ').length
			var totalWords = countByteLength(reHtml(text));
